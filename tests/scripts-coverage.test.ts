 ...
 * 
 * This file tests script behaviors with TypeScript typing and proper mocking.
 */
...
import { ProjectInfo, TechStack, DocumentationNeeds, ValidationResult, ValidationError, ValidationWarning } from '../src/types';
...
interface GenerateReportsBehavior {
  getDocumentFiles: () => string[];
  parseValidationResults: (results: Record<string, ValidationResult>) => ValidationStats;
  formatSummary: (stats: ValidationStats) => string;
  validateDocument: (filePath: string) => ValidationResult;
  validateAllDocuments: () => Record<string, ValidationResult>;
  extractDocumentMetadata: (filePath: string) => Record<string, string>;
  analyzeDocumentContent: (content: string) => DocumentAnalysis;
  generateReport: (filePath: string, validationResult: ValidationResult) => string;
}

interface InitializeBehavior {
  loadProjectDefaults: () => Record<string, any>;
  createProjectStructure: (projectConfig: Partial<ProjectInfo>) => string[];
  setupConfigFiles: (projectConfig: Partial<ProjectInfo>) => Record<string, string>;
  gatherBasicProjectInfo: (options: Record<string, any>) => ProjectInfo;
  recommendTechnologyStack: (projectInfo: ProjectInfo) => TechStack;
  assessDocumentationNeeds: (projectInfo: ProjectInfo, techStack: TechStack) => DocumentationNeeds;
  askFollowUpQuestions: (questions: string[], interviewAnswers: Record<string, string>) => Promise<Record<string, string>>;
  ...
  initializeProject: (options?: Record<string, any>) => Promise<{
    projectInfo: ProjectInfo;
    techStack: TechStack;
    documentationNeeds: DocumentationNeeds;
    interviewAnswers: Record<string, string>;
...
interface UpdateVersionsBehavior {
  findDocFiles: () => string[];
  parseVersionInfo: (content: string) => { currentVersion: string; type: string };
  incrementVersion: (version: string, type: string) => string;
  updateDocumentVersion: (content: string, newVersion: string) => string;
}

interface ValidateDocsBehavior {
  getDocumentFiles: () => string[];
  validateDocument: (filePath: string) => ValidationResult;
  validateAllDocuments: () => Record<string, ValidationResult>;
  findIssues: (content: string, docType: string) => { errors: ValidationError[]; warnings: ValidationWarning[] };
  displayValidationResult: (filePath: string, result: ValidationResult) => string;
  displayAllValidationResults: (results: Record<string, ValidationResult>) => string;
  displayValidationSummary: (results: Record<string, ValidationResult>) => string;
}

interface MockScriptBehaviors {
  generateReports: GenerateReportsBehavior;
  initialize: InitializeBehavior;
  updateVersions: UpdateVersionsBehavior;
  validateDocs: ValidateDocsBehavior;
}

const mockScriptBehaviors: MockScriptBehaviors = {
  generateReports: {
  ...
  initialize: {
    ...
    gatherBasicProjectInfo: (options): ProjectInfo => {
    ...
    recommendTechnologyStack: (projectInfo): TechStack => {
      return {
        recommended: ['React', 'Node.js', 'MongoDB'],
        selected: ['React', 'Node.js']
      };
    },
    assessDocumentationNeeds: (projectInfo, techStack): DocumentationNeeds => {
      const docNeeds: DocumentationNeeds = {
        prd: true,
        srs: true,
        sad: false,
        sdd: false,
...
jest.mock('child_process', () => ({
  ...
  exec: jest.fn((cmd, callback) => callback && callback(null, 'mocked exec output'))
}));
...
describe('initialize.js', () => {
  ...
  it('should create project structure', () => {
    const projectConfig: Partial<ProjectInfo> = {
      name: 'Test Project',
      description: 'A test project',
      type: 'WEB'
    };
