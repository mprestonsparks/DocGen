...
function loadProjectDefaults() {
  try {
    ...
    console.error(`Error loading project defaults: ${error.message}`);
  }
 ...
 */
async function gatherBasicProjectInfo() {
  ...
  try {
    ...
    console.error('Error gathering project information:', error.message);
    process.exit(1);
  }
}
 ...
 */
async function askFollowUpQuestions(questions, interviewAnswers) {
  ...
  for (const question of questions) {
    try {
      ...
      console.error('Error asking follow-up question:', error.message);
    }
  }
}
 ...
 */
async function recommendTechnologyStack(projectInfo) {
 ...
 */
async function selectTechnologies(recommendations) {
  ...
  try {
    ...
    console.error('Error selecting technologies:', error.message);
...
async function generateDocument(type, projectInfo, techStack, interviewAnswers) {
  ...
  try {
    ...
    const aiTemplatePath = path.join(__dirname, `../@claude-code/ai-${type}.md`);
