/**
 * MCP Adapter for GitHub Issues Server
 * 
...
const logFile = path.join(debugLogDir, 'github-mcp-adapter.log');
...
const REST_API_BASE = 'http://localhost:7867';
...
const methodMappings = {
  ...
  // GitHub methods mapped to our REST endpoints
  'github.getIssues': { endpoint: '/getIssues', method: 'POST' },
  'github.getIssue': { endpoint: '/getIssue', method: 'POST' },
  'github.createIssue': { endpoint: '/createIssue', method: 'POST' },
  'github.updateIssue': { endpoint: '/updateIssue', method: 'POST' },
  'github.addComment': { endpoint: '/addComment', method: 'POST' },
  'github.getImplementationStatus': { endpoint: '/getImplementationStatus', method: 'POST' },
  'github.getPullRequests': { endpoint: '/getPullRequests', method: 'POST' },
  'github.getPullRequest': { endpoint: '/getPullRequest', method: 'POST' },
  'github.createPullRequest': { endpoint: '/createPullRequest', method: 'POST' },
  'github.getFilesChanged': { endpoint: '/getFilesChanged', method: 'POST' }
};
...
async function handleInitialize(params, id) {
  ...
  try {
    ...
    return {
      serverInfo: {
        name: serverCapabilities?.name || 'GitHub Issues MCP',
...
function buildMethodsCapabilities(capabilities) {
  ...
  if (capabilities && capabilities.capabilities) {
    capabilities.capabilities.forEach(capability => {
      const methodName = `github.${capability.name}`;
      
      // Create method schema
      methods[methodName] = {
        description: capability.description || `GitHub ${capability.name} operation`,
        params: {
          type: 'object',
          properties: {}
        },
...
// Start the server
const PORT = process.env.MCP_PORT || 7866;
app.listen(PORT, () => {
  logger.info(`MCP GitHub adapter running on port ${PORT}`);
