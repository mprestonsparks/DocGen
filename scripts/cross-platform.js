 ...
 * 
 * Usage: node cross-platform.js <script-base-name> [args...]
 * Example: node cross-platform.js get-to-work --help
 */
...
function runPlatformScript(scriptBasePath, args) {
  // Special handling for docgen commands
  if (scriptBasePath === 'docgen') {
    if (platform.isWindows) {
      ...
      // On Unix, use the docgen.js script directly
      console.log(`Running docgen command directly`);
      const docgenScript = path.join(projectRoot, 'docgen.js');
      ...
      const result = spawnSync('node', [docgenScript, ...args], { 
        stdio: 'inherit',
        shell: true
      });
      process.exit(result.status || 0);
