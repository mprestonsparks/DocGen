 ...
 * 
 * This file tests the behavior of src/index.ts with proper TypeScript typing and mocking.
 */
...
import { ProjectInfo, TechStack, DocumentationNeeds, ValidationResult } from '../src/types';
    ...
    if (Array.isArray(questions)) {
      ...
      if (question.type === 'list') {
        ...
        return Promise.resolve({ 
          [question.name]: question.choices.slice(0, 2).map(
            c => typeof c === 'object' ? (c.value || c.name || c) : c
          ) 
        });
      } else {
...
interface MockIndexBehaviors {
  loadProjectDefaults: () => Record<string, any>;
  conductInterview: (options: InterviewOptions) => Promise<Record<string, any>>;
  gatherBasicProjectInfo: (options: InterviewOptions) => Promise<ProjectInfo>;
  recommendTechnologyStack: (projectInfo: ProjectInfo) => Promise<TechStack>;
  assessDocumentationNeeds: (projectInfo: ProjectInfo, techStack: TechStack) => Promise<DocumentationNeeds>;
  ...
  selectTechnologies: (recommendations: Record<string, string[]>) => Promise<string[]>;
  selectDocumentationTypes: (recommendations: Record<string, any>) => Promise<DocumentationNeeds>;
  ...
  createBasicTemplate: (type: string, projectInfo: ProjectInfo) => string;
}

const mockIndexBehaviors: MockIndexBehaviors = {
  loadProjectDefaults: () => ({
    schema_versions: { prd: '1.0.0', srs: '1.0.0' },
    document_versions: { prd: '1.0.0', srs: '1.0.0' },
    document_statuses: ['DRAFT', 'REVIEW', 'APPROVED'],
  ...
  conductInterview: async (options: InterviewOptions) => {
    if (options.list) {
    ...
    if (options.resume) {
      return { 
        ...
        projectInfo: { name: 'Resumed Project', type: 'WEB' } as ProjectInfo,
        techStack: { selected: ['React', 'Node.js'] } as TechStack,
        documentationNeeds: { prd: true, srs: true } as DocumentationNeeds
      };
    }
  ...
  gatherBasicProjectInfo: async (options: InterviewOptions): Promise<ProjectInfo> => {
  ...
  recommendTechnologyStack: async (projectInfo: ProjectInfo): Promise<TechStack> => {
  ...
  assessDocumentationNeeds: async (projectInfo: ProjectInfo, techStack: TechStack): Promise<DocumentationNeeds> => {
  ...
  askFollowUpQuestions: async (questions: string[], interviewAnswers: Record<string, string>): Promise<Record<string, string>> => {
  ...
  selectTechnologies: async (recommendations: Record<string, string[]>): Promise<string[]> => {
  ...
  selectDocumentationTypes: async (recommendations: Record<string, any>): Promise<DocumentationNeeds> => {
    return {
      prd: true,
      srs: true,
      sad: false,
...
describe('CLI Module', () => {
  ...
  it('should list sessions', async () => {
    ...
    expect(Array.isArray((result as any).sessions)).toBe(true);
    expect((result as any).sessions[0]).toHaveProperty('sessionId');
    expect((result as any).sessions[0]).toHaveProperty('projectName');
  });
  ...
  it('should resume a session', async () => {
    ...
    expect(result).toHaveProperty('projectInfo');
    expect((result as any).projectInfo).toHaveProperty('name', 'Resumed Project');
  });
