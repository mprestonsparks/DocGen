/**
 * Health check route for GitHub MCP Server
 */

import { Router, Request, Response } from 'express';
import { logger } from '../utils/logger';
import { getGitHubClientStatus } from '../services/github';
...
healthRouter.get('/', async (req: Request, res: Response) => {
  try {
    // Check GitHub API client status
    const githubStatus = await getGitHubClientStatus();
    ...
    const isHealthy = githubStatus.status === 'ok' && 
                      diskSpace.status === 'ok' && 
